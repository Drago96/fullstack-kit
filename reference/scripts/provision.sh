...
# Cloud Run, the Workload Identity deploy identity, the api-runtime service account the
# service runs as), Vercel, Sentry, Gemini and Resend. The API's five secrets —
# DATABASE_URL, AUTH_SECRET, SENTRY_DSN, RESEND_API_KEY, GOOGLE_GENERATIVE_AI_API_KEY —
# go into Google Secret Manager, where only the runtime service account can read them and
# the Cloud Run revision holds nothing but references. Every other value lands in this
# repository's GitHub secrets and variables, where the deploy workflows read them. The
# Loops run from your terminal (ADR 0009), so nothing here provisions a Claude token.
...
# Nothing here writes a secret to disk or prints one: every captured secret goes straight
# to `gh secret set`, or to `set_gcp_secret` for the API's own five, which pipes it into
# `gcloud secrets versions add`. Local development needs none of them — .env.example
