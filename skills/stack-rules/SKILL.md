...
- **Need a primitive that is not there** (textarea, select, dialog, table)? `pnpm dlx shadcn add <name>` from `apps/web/`, and add nothing the page does not use — an unused export fails `pnpm knip`. The notes body is today's one exception: a plain `<textarea>` carrying `Input`'s classes, until a second multiline field earns the primitive. Only reach for an icon set (`lucide-react`) when a component you added actually imports one.
- **Compose, do not restyle**: `<Button asChild><Link …></Button>` keeps the anchor a link, so an E2E Test finds it by role. A form field is `<Label htmlFor>` + `<Input id>` + a `<span role="alert">` for the error; a failure the whole form reports is `<Alert variant="destructive"><AlertDescription>`. `apps/web/src/app/[locale]/login/page.tsx` is the shape end to end.
- E2E Tests locate by role, label and heading, never by class, so the class list is free to change but the roles, labels and headings are not.
