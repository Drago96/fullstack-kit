...
Where each variable comes from: GitHub **variables** hold the non-secret config the Cloud Run revision needs (`GCP_PROJECT_ID`, `GCP_REGION`, `GCP_ARTIFACT_REPOSITORY`, `GCP_WORKLOAD_IDENTITY_PROVIDER`, `GCP_DEPLOY_SERVICE_ACCOUNT`, `CLOUD_RUN_SERVICE`, `API_URL`, `WEB_URL`, `EMAIL_TRANSPORT`, `LLM_PROVIDER`). GitHub **secrets** hold `DATABASE_URL`, `AUTH_SECRET`, `SENTRY_DSN`, `RESEND_API_KEY`, `GOOGLE_GENERATIVE_AI_API_KEY` for the API, and `VERCEL_TOKEN`, `VERCEL_ORG_ID`, `VERCEL_PROJECT_ID` for the web deploy. Web's own two variables, `API_URL` and `NEXT_PUBLIC_SENTRY_DSN`, live in Vercel's production environment, not in GitHub, because `vercel build` reads them from there. `apps/api/.env.example` and `apps/web/.env.example` list every variable each app validates, one line of purpose each.

The API's secrets reach Cloud Run as plain environment variables on the revision, readable by anyone with `run.services.get` on the GCP project. That is the trade for one fewer service to provision on a single-owner Project; swap those rows for `deploy-cloudrun`'s `secrets:` input backed by Secret Manager when a Project grows a second person.
