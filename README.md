...
A Project's pull requests are gated by its own `CI` check — `.github/workflows/ci.yml`, the same steps the Kit runs against `reference/`, carried along by `scaffold` and kept in step by the Sync Loop.
