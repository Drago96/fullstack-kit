...
# Cloud Run, the Workload Identity deploy identity), Vercel, Sentry, Gemini, Resend,
# and the Claude subscription token — and leaves every value in this repository's
# GitHub secrets and variables, where the deploy workflows and the Loops read them.
...
set_secret SENTRY_PROJECT "$SLUG-api"
...
# ── 11 ────────────────────────────────────────────────────────────────────
stage "Claude and GitHub tokens for the Loops"
say "The Error, Sync and Review Loops run on your Claude subscription, not the API."
step "In another terminal run: claude setup-token"
step "It prints a token. Copy it — this wizard never prints it or puts it on disk."
ask_secret CLAUDE_CODE_OAUTH_TOKEN "Paste the Claude subscription token:"
set_secret CLAUDE_CODE_OAUTH_TOKEN "$CLAUDE_CODE_OAUTH_TOKEN"
open_url "https://github.com/settings/personal-access-tokens/new"
step "Fine-grained PAT on $REPO with contents, pull-requests and workflows: write."
step "Without it a Loop's push does not re-trigger CI, so its PR sits untouched."
ask_secret REVIEW_LOOP_GH_TOKEN "Paste the GitHub PAT:"
